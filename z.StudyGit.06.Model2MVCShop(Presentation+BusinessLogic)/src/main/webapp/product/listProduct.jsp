					...
					<td align="right">
						<c:if test="${ menu eq 'manage'}" >
							<select name="searchCondition" id="searchCondition" class="ct_input_g" style="width: 80px">
								<option value="0" ${ search.searchCondition eq '0' ? 'selected' : '' }>검색조건 선택</option>
							...
							<td align="left">
								<c:choose>
									<c:when test="${ product.proTranCode eq '0' }">
										<a href="/${ menu eq 'manage' ? 'updateProductView' : 'getProduct'}.do?prodNo=${ product.prodNo }&menu=${menu}">${ product.prodName }</a>
									</c:when>
									<c:otherwise>
										${ product.prodName }
									</c:otherwise>
